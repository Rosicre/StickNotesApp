...
# Isso vai carregar a configuração do arquivo Config
# e a variável DATABASE_URL pode ser substituída de forma segura.
...
def get_notes():
    try:
        ...
        return jsonify([{"id": note.id, "content": note.content, "category": note.category} for note in notes])
...
def add_note():
    try:
        ...
        db.session.commit()
        return jsonify({"message": "Nota adicionada!", "note": {"id": new_note.id, "content": new_note.content, "category": new_note.category}})
    except Exception as e:
